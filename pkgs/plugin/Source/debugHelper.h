...
void dbg_printPixels(ai::uint8 *pixelData, ai::uint32 totalPixels, ai::uint32 pixelStride)
{
    std::cout << "Current Pixel Values (first 200 pixels):" << std::endl;
    for (ai::uint32 i = 0; i < std::min(static_cast<ai::uint32>(200), totalPixels) * pixelStride; i += pixelStride)
    {
...
void dbg_printRasterizeSettings(const AIRasterizeSettings &settings)
{
    ...
    {
        if (settings.options & kRasterizeOptionsDoLayers)
            optionsStr += "DoLayers|";
        ...
        if (settings.options & kRasterizeOptionsHinted)
            optionsStr += "Hinted|";
    ...
    std::cout << "  Resolution: " << (settings.resolution <= 0 ? "Default 72 dpi" : std::to_string(settings.resolution) + " dpi") << std::endl;
    std::cout << "  Antialiasing: " << (settings.antialiasing < 2 ? "Off" : "On") << " (" << settings.antialiasing << ")" << std::endl;
    std::cout << "  Options: " << optionsStr << " (" << settings.options << ")" << std::endl;
    std::cout << "  Color Conversion Purpose: " << settings.ccoptions.purpose << std::endl;
    std::cout << "  Preserve Spot Colors: " << (settings.preserveSpotColors ? "Yes" : "No") << " (" << settings.preserveSpotColors << ")" << std::endl;
    std::cout << std::endl;
}
